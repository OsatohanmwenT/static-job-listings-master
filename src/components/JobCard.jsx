...
export default function JobCard({ logo, company, info, featured, position, role, contract, location, languages, postedAt, level, tools, filterSelect, filter }) {
    return(
        <div className={`flex flex-1 flex-wrap relative justify-between items-center max-sm:flex-col max-sm:items-start  max-sm:gap-4 w-full p-6 max-sm:p-4 max-sm:pt-8 ${info && featured ? "border-l-[3px] border-Desaturated-Dark-Cyan": ""}
           bg-white mt-6 max-sm:mt-10 rounded-sm shadow-3xl`}>
            <div className="flex gap-4 items-center ">
                ...
                <div>
                    <div className="flex flex-1 items-center gap-2">
                        <p className="text-Desaturated-Dark-Cyan text-sm font-bold">{company}</p>
                        {info && 
            ...
            <div className="flex gap-2 flex-wrap items-center">
                ...
                {languages.map((language, index) => (
                    <FilterButton key={index} text={language} filterSelect={filterSelect} />
                ))}
